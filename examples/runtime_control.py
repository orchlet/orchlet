...
async def main(journal_path=None):
    started = asyncio.Event()

    @task(context=True)
    async def wait_for_input(task_ctx, label):
    ...
    async with runtime:
        ...
        logger.info(
            f"New task state: {extra.snapshot.state.value}; updated metrics: {extra.snapshot.metrics}"
        )
